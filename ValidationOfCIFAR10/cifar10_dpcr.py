...
import dpcrpy   # import for dpcr framework and models
from opacus_dpcr.dpcr_engine import DPCREngine  # import for Opacus-DPCR
...
# flake8: noqa: C901
def main(dpcr_model='NoDPCR',sigma=1.5):
    args = parse_args()
    args.sigma = sigma
    print(f"> You set sigma as {args.sigma}.")
    ...
    if not args.disable_dp:
        ...
        if dpcr_model!='NoDPCR':
            print(f"> You select DPCR with {dpcr_model}...")
            # select a DPCR model for private training
            bkSz = [11, 8]
            dpcrMdInfo = {"SimpleMech": {"genBk": lambda x: dpcrpy.SimpleMech()},
                        "TwoLevel": {"genBk": lambda x: dpcrpy.TwoLevel(kOrder=48)},
                        "BinMech": {"genBk": lambda x: dpcrpy.BinMech(kOrder=bkSz[x-1])},
                        "FDA": {"genBk": lambda x: dpcrpy.FDA(kOrder=bkSz[x-1])},
                        "BCRG": {"genBk": lambda x: dpcrpy.BCRG(kOrder=bkSz[x-1])},
                        "ABCRG": {"genBk": lambda x: dpcrpy.ABCRG(kOrder=bkSz[x-1])},
                        }
            privacy_engine = DPCREngine(secure_mode=False, dpcrGenerator=dpcrMdInfo[dpcr_model]['genBk'])
        else:
            # original Opacus model
            print('You select traditional DP learning...')
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="This parameter is meaningless, just to make the script executable under Jupyter.",
    )
